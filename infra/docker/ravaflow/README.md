...
Consequence for this corridor, measured and not hypothetical: **87.4%** of the Lhende / Bhote
Koshi / Trishuli thalweg is below **4.57 degrees** — the slope at which a Coulomb friction
coefficient of 0.08 can no longer drive the flow — so a coefficient above 0.08 cannot sustain
motion over most of it. The measurement is `thalweg_fraction_below_mu_threshold` in
`reports/runout/terrain.json`, generated by `scripts/runout_terrain_report.py`. A single-phase Voellmy rheology therefore stops far short of
