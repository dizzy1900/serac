...
def render(repo: Path) -> str:
    ...
    add(
        ...
        "**A fix exists and this model was not trained under it** (2026-09-10): "
        "`serac.models.discriminator.balance` equalises the realised receiver count within each "
        "event group and takes the leak metric from ROC-AUC 0.6077 to 0.5083 (0.5008 once the 34 "
        "orphaned windows are excluded), at a cost of 8.38 -> 5.72 receivers on a positive. "
        "Run `serac models receiver-balance` for the current numbers. Every score in this card "
        "predates the rule, so the honest reading is unchanged: how much of this skill is "
        "receiver density is still unknown, and now measurable.\n"
